...
function portalResponse(title: string, body: string, scriptPath: string): Response {
  const html = `<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>${PORTAL_STYLE}</style></head><body>${body}<script src="${scriptPath}" defer></script></body></html>`;
...
export function loginPortalPage(): Response {
  return portalResponse(
    'CHRTV — Đăng nhập M3U',
    `<main><div class="top"><div><div class="brand">CHRTV</div><div class="muted">Playlist riêng & quản lý thiết bị</div></div></div>
<section class="card" id="login-card"><h1>Đăng nhập M3U an toàn</h1><p class="hint">Mật khẩu được gửi bằng POST và không nằm trong URL playlist. Link session có thể thu hồi bất cứ lúc nào.</p>
<form id="login-form"><div class="grid"><label>Tên đăng nhập<input name="username" autocomplete="username" required maxlength="128"></label><label>Mật khẩu<input name="password" type="password" autocomplete="current-password" required maxlength="256"></label><label>Tên thiết bị<input name="device_name" placeholder="TV phòng khách" maxlength="80"></label></div><div class="row" style="margin-top:14px"><label style="display:flex;min-width:auto"><input name="replace_oldest" type="checkbox"> Thu hồi thiết bị cũ nhất nếu đã đủ giới hạn</label><button class="primary" type="submit">Tạo link M3U</button></div></form><p id="login-error" class="error"></p></section>
<section class="card hidden" id="playlist-card"><h2>Link playlist của thiết bị này</h2><div id="playlist-url" class="url"></div><div class="row" style="margin-top:12px"><button id="copy-url">Sao chép link</button><button id="forget" class="danger">Xóa token khỏi trình duyệt</button></div></section>
<section class="card hidden" id="sessions-card"><div class="top"><div><h2>Thiết bị / phiên đăng nhập</h2><div id="account-name" class="muted"></div></div><button id="refresh-sessions">Làm mới</button></div><div class="scroll"><table><thead><tr><th>Thiết bị</th><th>IP tạo / gần nhất</th><th>Hoạt động</th><th>Trạng thái</th><th></th></tr></thead><tbody id="sessions-body"></tbody></table></div><p id="sessions-error" class="error"></p></section></main>`,
    '/ui/login.js',
  );
}
...
export function adminDashboardPage(): Response {
  return portalResponse(
    ...
    `<main><div class="top"><div><div class="brand">CHRTV ADMIN</div><div class="muted">Security, users, sessions & devices</div></div><div class="row"><input id="admin-token" type="password" autocomplete="current-password" placeholder="ADMIN_TOKEN"><button id="connect" class="primary">Kết nối</button><button id="logout">Xóa token</button></div></div><p id="admin-error" class="error"></p>
<div id="dashboard" class="hidden"><div class="row" style="margin-bottom:14px"><button id="refresh">Làm mới</button><button id="sync">Sync playlist</button></div><section class="grid" id="stats"></section>
...
<section class="card tab" id="tab-events"><h2>Đăng nhập và tải playlist gần đây</h2><div class="scroll"><table><thead><tr><th>Thời gian</th><th>User</th><th>IP</th><th>Sự kiện</th><th>Route</th><th>Kết quả</th></tr></thead><tbody id="events-body"></tbody></table></div></section>
<section class="card tab hidden" id="tab-sessions"><h2>Phiên M3U có thể thu hồi</h2><div class="scroll"><table><thead><tr><th>User</th><th>Thiết bị</th><th>IP</th><th>Last seen</th><th>Hết hạn</th><th>Trạng thái</th><th></th></tr></thead><tbody id="admin-sessions-body"></tbody></table></div></section>
<section class="card tab hidden" id="tab-bans"><h2>IP hash đang bị ban</h2><div class="scroll"><table><thead><tr><th>Hash</th><th>Lý do</th><th>Last seen</th><th>Hết hạn</th><th>Hits</th><th></th></tr></thead><tbody id="bans-body"></tbody></table></div></section>
<section class="card tab hidden" id="tab-users"><h2>Người dùng</h2><div class="scroll"><table><thead><tr><th>ID</th><th>Username</th><th>Status</th><th>Max sessions</th><th>Hết hạn</th><th></th></tr></thead><tbody id="users-body"></tbody></table></div></section>
<section class="card tab hidden" id="tab-devices"><h2>Thiết bị access key / MAC</h2><div class="scroll"><table><thead><tr><th>ID</th><th>Key</th><th>MAC</th><th>First seen</th><th>Last seen</th><th>Status</th><th></th></tr></thead><tbody id="devices-body"></tbody></table></div></section></div></main>`,
    '/ui/admin.js',
  );
}

const LOGIN_JS = String.raw`
const $=s=>document.querySelector(s);const escDate=v=>v?new Date(v*1000).toLocaleString():'Không hết hạn';
...
async function sessions(){if(!token)return;try{const data=await api('/api/account/sessions');$('#sessions-body').textContent='';$('#account-name').textContent=data.user.username+' · tối đa '+data.user.max_connections+' phiên';for(const s of data.sessions){const tr=document.createElement('tr');td(tr,s.device_name||('Session '+s.token_prefix));td(tr,(s.ip_address||'—')+' / '+(s.last_ip||'—'));td(tr,escDate(s.last_seen));td(tr,s.id===data.current_session_id?s.status+' (hiện tại)':s.status);const a=td(tr,'');const b=document.createElement('button');b.textContent='Thu hồi';b.className='danger';b.disabled=s.status!=='active';b.onclick=async()=>{await api('/api/account/sessions/'+s.id,{method:'DELETE'});if(s.id===data.current_session_id){token='';sessionStorage.clear();location.reload()}else sessions()};a.append(b);$('#sessions-body').append(tr)}$('#sessions-card').classList.remove('hidden');$('#sessions-error').textContent=''}catch(e){$('#sessions-error').textContent=e.message}}
...
$('#copy-url').onclick=async()=>{await navigator.clipboard.writeText(playlist);$('#copy-url').textContent='Đã sao chép'};$('#refresh-sessions').onclick=sessions;$('#forget').onclick=()=>{sessionStorage.clear();location.reload()};show();
`;
