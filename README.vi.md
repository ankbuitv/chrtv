...
- **Rolling list theo người xem**: link playlist `/p/{session}.m3u` vẫn cố định, còn toàn bộ URL kênh bên trong gắn với một lease riêng được nhận diện bằng user/session/MAC/access-key cùng IP và User-Agent đã HMAC (không lưu username/IP thô trong bảng lease). Trong lúc player còn refresh manifest, tải lại list vẫn nhận đúng URL cũ để không mất buffer. Sau **60 giây không còn request manifest**, lần tải list kế tiếp tự sinh generation mới và URL cũ trả `410 TOKEN_EXPIRED`; còn player chỉ tạm ngưng (pause/seek/mất mạng) rồi quay lại thì được **hồi sinh đúng generation cũ**, không bao giờ chết vì pause. Web player `/xem` (hls.js) tự nhận diện `410`/`403` và tự cấp lại link kênh mới. Nếu bảng `viewer_leases` chưa có (chưa chạy migration) thì playlist tự hạ cấp về token không lease thay vì lỗi. IP chỉ dùng tách viewer lease, không khóa cứng playback khi `TOKEN_BINDING` không chứa `ip`, nên đổi IP/CGNAT không cắt stream đang chạy.
